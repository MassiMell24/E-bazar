      ...
      <?php foreach ($purchased as $p): ?>
        <li>
          ...
          <div style="margin-top:6px">
            <?php if (empty($p['buyer_confirmed_reception'])): ?>
              <form method="post" action="/e-bazar/index.php?url=dashboard/confirmReceived/<?php echo (int)$p['id']; ?>" onsubmit="showConfirm('Confirmer la réception du produit ?', this); return false;" style="display:inline;margin-right:8px">
            ...
            <?php endif; ?>
            <form method="post" action="/e-bazar/index.php?url=dashboard/deletePurchasedAd/<?php echo (int)$p['id']; ?>" onsubmit="showConfirm('Supprimer définitivement cet article de votre historique ?', this); return false;" style="display:inline">
              <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION['csrf_token'] ?? ''); ?>">
