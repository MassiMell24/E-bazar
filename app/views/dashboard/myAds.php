    ...
    <?php else: ?>
        ...
        <?php foreach ($sold as $a): ?>
            <li style="display:flex;align-items:center;justify-content:space-between;gap:12px;padding:10px 12px;">
                ...
                <form method="post" action="/e-bazar/index.php?url=dashboard/deleteSoldAd/<?php echo (int)$a['id']; ?>" style="margin:0" onsubmit="return confirm('Supprimer cet article vendu ?');">
        ...
        <?php foreach ($purchased as $a): ?>
            <li style="display:flex;align-items:center;justify-content:space-between;gap:12px;padding:10px 12px;">
                ...
                <div style="display:flex;gap:8px">
                    <?php if (empty($a['buyer_confirmed_reception'])): ?>
                        <form method="post" action="/e-bazar/index.php?url=dashboard/confirmReceived/<?php echo (int)$a['id']; ?>" style="margin:0" onsubmit="return confirm('Confirmer la réception de cet article ?');">
                            <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION['csrf_token'] ?? ''); ?>">
                            <button type="submit" class="btn" style="padding:6px 12px;font-size:12px;background:#2f855a;color:#fff">✓ Reçu</button>
                        </form>
                    <?php endif; ?>
                    <form method="post" action="/e-bazar/index.php?url=dashboard/deletePurchasedAd/<?php echo (int)$a['id']; ?>" style="margin:0" onsubmit="return confirm('Supprimer cet article de votre historique ?');">
                        <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION['csrf_token'] ?? ''); ?>">
