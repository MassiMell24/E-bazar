				...
				<li style="display:flex;align-items:center;justify-content:space-between;gap:12px;padding:10px 12px;">
						...
						<div style="display:flex;gap:6px">
								<?php if (!empty($_SESSION['user_id']) && ($_SESSION['user_id'] == $ad['owner_id'])): ?>
										<a class="btn secondary" href="/e-bazar/index.php?url=ad/edit/<?php echo (int)$ad['id']; ?>" style="padding:6px 12px;font-size:12px;text-decoration:none">Éditer</a>
								<?php endif; ?>

								<?php if (!empty($_SESSION['user_id']) && ($_SESSION['user_id'] == $ad['owner_id'] || !empty($_SESSION['is_admin']))): ?>
								<form method="post" action="/e-bazar/index.php?url=ad/delete/<?php echo (int)$ad['id']; ?>" style="margin:0" onsubmit="showConfirm('⚠️ Supprimer définitivement l\'annonce « <?php echo htmlspecialchars($ad['title']); ?> » ?\n\nCette action est irréversible.', this); return false;">
		...
		<?php if (!empty($totalPages) && $totalPages > 1): ?>
			<div class="pagination" style="margin-top:24px;">
				<?php 
					...
					if ($currentPage > 1):
						$prevQuery = http_build_query(['url' => 'ad', 'category' => $selectedCategory, 'sort' => $_GET['sort'] ?? null, 'page' => $currentPage - 1]);
				?>
					<a class="pagination-btn" href="/e-bazar/index.php?<?php echo htmlspecialchars($prevQuery); ?>">‹</a>
				<?php endif; ?>

				<div class="pagination-pages">
					<?php for ($p = 1; $p <= $totalPages; $p++):
							if ($totalPages > 7 && $p > 2 && $p < $currentPage - 1):
								if ($p == 3): echo '<span class="pagination-ellipsis">…</span>'; endif;
								continue;
							endif;
							if ($totalPages > 7 && $p > $currentPage + 1 && $p < $totalPages - 1):
								if ($p == $currentPage + 2): echo '<span class="pagination-ellipsis">…</span>'; endif;
								continue;
							endif;

							$query = http_build_query(['url' => 'ad', 'category' => $selectedCategory, 'sort' => $_GET['sort'] ?? null, 'page' => $p]);
							$isActive = ($p == $currentPage);
					?>
						<a class="pagination-page<?php echo $isActive ? ' is-active' : ''; ?>" href="/e-bazar/index.php?<?php echo htmlspecialchars($query); ?>"><?php echo $p; ?></a>
					<?php endfor; ?>
				</div>

				<?php 
					if ($currentPage < $totalPages):
						$nextQuery = http_build_query(['url' => 'ad', 'category' => $selectedCategory, 'sort' => $_GET['sort'] ?? null, 'page' => $currentPage + 1]);
				?>
					<a class="pagination-btn" href="/e-bazar/index.php?<?php echo htmlspecialchars($nextQuery); ?>">›</a>
				<?php endif; ?>

				<span class="pagination-info">Page <?php echo $currentPage; ?> sur <?php echo $totalPages; ?></span>
			</div>
		<?php endif; ?>
	</div>
</main>
