<main>
  <div class="card">
    ...
    <form method="post" action="/e-bazar/index.php?url=ad/update/<?php echo (int)$ad['id']; ?>" enctype="multipart/form-data" onsubmit="return confirm('Êtes-vous sûr de vouloir enregistrer les modifications ?');">
        ...
        <script>
            (function(){
                ...
                // Confirm when a checkbox is checked to remove image
                const checkboxes = form.querySelectorAll('input[type="checkbox"][name="remove_images[]"]');
                checkboxes.forEach(function(cb){
                    cb.addEventListener('change', function(e){
                        if (cb.checked) {
                            const ok = confirm('Voulez-vous vraiment supprimer cette image ?');
                            if (!ok) {
                                cb.checked = false;
                            } else {
