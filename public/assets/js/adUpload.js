...
(function(){
  ...
  form.addEventListener('submit', function(e){
    ...
    if (newCount > 0) {
      var fileNames = files.map(function(f){ return f.name + ' (' + humanSize(f.size) + ')'; });
      var msg = 'Vous allez ajouter ' + newCount + ' nouvelle(s) photo(s)';
      if (existingCount > 0) msg += ' (total: ' + totalCount + ' photos après modification)';
      msg += ':\n- ' + fileNames.join('\n- ');
      msg += '\n\nConfirmez-vous ?';
      if (!confirm(msg)) {
        e.preventDefault();
